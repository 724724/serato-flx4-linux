...
if [[ -n "$MUSIC_DIR" ]]; then
    ...
    MUSIC_DRIVE_LINK="$WINEPREFIX/dosdevices/m:"
    MUSIC_DIR_WIN="M:\\"
    if [[ $DRY_RUN -eq 0 ]]; then
        if [[ -L "$MUSIC_DRIVE_LINK" ]]; then
            [[ "$(readlink -f "$MUSIC_DRIVE_LINK")" == "$MUSIC_DIR" ]] ||
                die "Wine M: already points somewhere else: $(readlink "$MUSIC_DRIVE_LINK")"
        elif [[ -e "$MUSIC_DRIVE_LINK" ]]; then
            die "Wine M: already exists and is not a symlink: $MUSIC_DRIVE_LINK"
        else
            ln -s "$MUSIC_DIR" "$MUSIC_DRIVE_LINK"
        fi
        ...
        info "[DRY] would map Wine M: → $MUSIC_DIR"
